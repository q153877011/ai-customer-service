...
> **安全说明**：`APP_KEY` 不带 `NEXT_PUBLIC_` 前缀，仅在服务端 API Routes 中可读，不会暴露给浏览器。`.env` 已加入 `.gitignore`，不会被提交到版本库。
...
### Vercel / EdgeOne Pages（推荐）

1. Fork 本仓库到你的 GitHub
2. 在 Vercel / EdgeOne Pages 中导入该仓库
3. 在控制台的 Environment Variables 中配置 `APP_KEY` 和 `API_URL`
4. 点击部署，完成
...
ENV API_URL=https://api.dify.ai/v1
EXPOSE 3000
CMD ["npm", "start"]
```
